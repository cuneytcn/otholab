...
const Header = () => {
   ...
   return (
      <div className="absolute top-0 left-0 w-full has-user-valid: z-50">
         <header
               ...
               <div className="flex justify-between items-center py-5">
                  ...
                  <div className="flex justify-center items-center">
                     <button
                        ...
                        className={clsx(
                           'size-10 lg:size-11 flex justify-center items-center flex-col cursor-pointer transition-all duration-300',
                           {
                        ...
                        <span
                           className={clsx(
                              'inline-block w-full h-px rounded-full bg-neutral-800 transition-all duration-300',
                              {
         ...
         <div
            id="mobile-menu"
            className={clsx(
               'fixed inset-0 w-full h-full z-50 bg-white transition-all duration-300',
               {
                     ...
                     {navigation.map((item) => (
                        <li
                           ...
                           <Link
                              ...
                              onClick={() =>
                                 dispatch(
                                    toggleModal({ modalName: 'mobileMenu' })
                                 )
                              }
                              ...
                              {item.name === 'Courses' ? (
                                 <>
                                    Courses{' '}
                                    <MoveRight className="inline lg:size-10 stroke-[1.5] ml-1" />
                                 </>
                              ) : (
                                 item.name
                              )}
                              {pathname === item.href && (
                                 <div className="absolute top-3 left-0 flex">
                                    {item.name.split('').map((char, index) => (
                                       <span
                                          key={index}
                                          className="block opacity-50 hover:-translate-y-1 transition-all duration-300">
                                          {char}
                                       </span>
                                    ))}
                                 </div>
